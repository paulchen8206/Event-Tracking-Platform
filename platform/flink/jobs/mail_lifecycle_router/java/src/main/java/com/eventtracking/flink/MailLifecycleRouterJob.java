...
public class MailLifecycleRouterJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        OffsetsInitializer offsetsInitializer = "earliest".equalsIgnoreCase(startingOffsets)
        ...
        DataStream<String> input = env.fromSource(source, WatermarkStrategy.noWatermarks(), "raw-lifecycle-events");

        SingleOutputStreamOperator<String> internalStream = input.process(new ProcessFunction<>() {
            @Override
            public void processElement(String value, ProcessFunction<String, String>.Context ctx, Collector<String> out)
                ...
                out.collect(value);
                if (isCustomerFacingEvent(eventType) && !root.path("tenant_id").asText("").isBlank()) {
    ...
    private static boolean isCustomerFacingEvent(String eventType) {
        Set<String> customerEvents = Set.of(
                "mail.requested",
                "mail.dispatched",
                "mail.delivered",
                "mail.opened",
    ...
    private static String buildHealthEvent(String reason, String sourcePayload) {
        return "{"
                ...
                + "\"job_type\":\"flink\","
                + "\"source_payload\":" + OBJECT_MAPPER.valueToTree(sourcePayload)
                + "}}";
    }
