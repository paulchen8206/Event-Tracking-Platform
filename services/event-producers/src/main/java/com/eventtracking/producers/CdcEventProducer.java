 ...
 * consumes. The generated payloads match the Debezium PostgreSQL connector wire format,
 * allowing the full CDC → lifecycle-raw pipeline to run locally without a real Postgres
 * instance or Debezium deployment.
 *
 * <p>Envelope shape:
 * <pre>
 * {
 ...
 *   <li>{@code CDC_TARGET_TOPIC} — default {@code dbz.postgres.mail.public.mail_events}</li>
...
public class CdcEventProducer {
    ...
    private static final List<String> STATUSES = List.of(
            "requested", "queued", "dispatched", "delivered",
    ...
    public void run(String bootstrapServers, String topic, int eventCount,
        ...
        LOG.info("CDC producer starting — topic={} count={} rps={}", topic, eventCount, eventsPerSecond);
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(buildProducerConfig(bootstrapServers))) {
            for (int i = 0; i < eventCount; i++) {
                ...
                producer.send(new ProducerRecord<>(topic, messageId, payload), (meta, ex) -> {
                    if (ex != null) {
