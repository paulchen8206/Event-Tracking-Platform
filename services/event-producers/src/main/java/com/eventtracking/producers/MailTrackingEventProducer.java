 ...
 *
 * <p>Produces events toward the topic that {@code OperationalMailTrackingRouterJob} consumes. The
 * generated records mirror what an AWS DynamoDB Streams Lambda bridge would emit after normalising
 * a DynamoDB change image into the platform's canonical JSON shape.
 *
 * <p>Each record is a plain JSON string (no Avro — the Flink job uses {@code SimpleStringSchema})
 * containing all the fields the router expects at the top level, plus an inline {@code
 * payload_json} object carrying the operational detail fields.
 *
 * <p>Environment variables:
 *
 * <ul>
 *   <li>{@code KAFKA_BOOTSTRAP_SERVERS} — default {@code localhost:9092}
 *   <li>{@code MAIL_TRACKING_TARGET_TOPIC} — default {@code evt.mail.operational.raw}
...
public class MailTrackingEventProducer {
  ...
  private static final List<String> STATUSES =
      List.of("queued", "dispatched", "delivered", "deferred", "bounced", "failed", "complained");

  private static final List<String> PROCESSING_STAGES =
      List.of("ingestion", "routing", "delivery", "post-delivery", "retry");

  private static final List<String> PROVIDERS =
      Arrays.asList("sendgrid", "mailgun", "ses", "postmark", null);

  private static final List<String> ERROR_CODES =
      Arrays.asList(
          null,
          null,
          null,
          null, // most events have no error
          "550",
          "421",
          "452",
          "smtp_timeout" // realistic SMTP error codes
          );
  ...
  public void run(
      ...
      throws Exception {
    LOG.info(
        "Mail-tracking producer starting — topic={} count={} rps={}",
        topic,
        eventCount,
        eventsPerSecond);
    ...
    try (KafkaProducer<String, String> producer =
      ...
      for (int i = 0; i < eventCount; i++) {
        ...
        producer.send(
            new ProducerRecord<>(topic, messageId, record),
            (meta, ex) -> {
              if (ex != null) {
