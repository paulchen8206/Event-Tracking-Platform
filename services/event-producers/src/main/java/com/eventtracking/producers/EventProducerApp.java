 ...
 *
 * <p>Reads all configuration from environment variables so the producers can be run inside the
 * local Docker Compose development stack or directly from the shell.
 *
...
public class EventProducerApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    LOG.info(
        "Event producer suite — mode={} count={} rps={} tenants={} continuous={} batchIntervalMs={}"
            + " topicWaitTimeoutMs={}",
        mode,
        eventCount,
        eventsPerSecond,
  ...
  private static void runBatch(
    ...
    switch (mode) {
      ...
      case "mail-tracking" ->
          runMailTracking(
              bootstrapServers, mailTrackingTopic, eventCount, eventsPerSecond, tenantCount);
      case "both" -> {
  ...
  private static void runCdc(
      String bootstrapServers, String topic, int count, int rps, int tenantCount) throws Exception {
    new CdcEventProducer().run(bootstrapServers, topic, count, rps, tenantCount);
  }

  private static void runMailTracking(
      String bootstrapServers, String topic, int count, int rps, int tenantCount) throws Exception {
    new MailTrackingEventProducer().run(bootstrapServers, topic, count, rps, tenantCount);
  }

  private static List<String> requiredTopics(
      String mode, String cdcTopic, String mailTrackingTopic) {
    return switch (mode) {
  ...
  private static void waitForRequiredTopics(
      String bootstrapServers, List<String> requiredTopics, int timeoutMs, int pollIntervalMs)
      throws Exception {
    ...
    try (AdminClient adminClient = AdminClient.create(props)) {
      while (Instant.now().isBefore(deadline)) {
        try {
          ...
          LOG.info(
              "Waiting for topics {}. Currently available: {}", requiredTopics, existingTopics);
        } catch (Exception e) {
    ...
    throw new IllegalStateException(
        "Timed out waiting for required Kafka topics: " + requiredTopics);
  }
  ...
  private static int intEnv(String key, int fallback) {
    ...
    try {
      ...
      LOG.warn(
          "Invalid integer value '{}' for env var '{}', using default {}", value, key, fallback);
      return fallback;
    }
  }
