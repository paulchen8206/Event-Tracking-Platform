...
public class CanonicalTopicToIcebergJob implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        String landingTableName = properties.getIceberg().getCatalogName()
                + "."
                + properties.getIceberg().getNamespace()
                + "."
                + properties.getIceberg().getTable();
        String tenantDimensionTable = qualifiedTableName(properties.getTableau().getTenantDimensionTable());
        String eventTypeDimensionTable = qualifiedTableName(properties.getTableau().getEventTypeDimensionTable());
        String dailyFactTable = qualifiedTableName(properties.getTableau().getDailyFactTable());

        ensureIcebergTables(spark, landingTableName, tenantDimensionTable, eventTypeDimensionTable, dailyFactTable);

        Dataset<Row> kafkaEvents = spark.readStream()
                .format("kafka")
        ...
        StructType canonicalSchema = new StructType()
        ...
        Dataset<Row> canonicalEvents = kafkaEvents
        ...
        DataStreamWriter<Row> writer = canonicalEvents.writeStream()
                .option("checkpointLocation", properties.getCheckpoint().getLocation())
            .foreachBatch((VoidFunction2<Dataset<Row>, Long>) (batch, batchId) -> processBatch(
                spark,
                batch,
                batchId,
                landingTableName,
        ...
        LOGGER.info("Started customer analytics consumer from {} into landing table {} with Tableau assets {}, {}, {}",
            properties.getKafka().getTopic(), landingTableName, tenantDimensionTable, eventTypeDimensionTable, dailyFactTable);
        ...
        refreshTableauAssets(spark, landingTableName, tenantDimensionTable, eventTypeDimensionTable, dailyFactTable);
        batch.unpersist();

        LOGGER.info("Processed micro-batch {} into landing and Tableau-serving Iceberg tables", batchId);
        }
    ...
    private SparkSession buildSparkSession() {
        SparkSession.Builder builder = SparkSession.builder()
            ...
            .config("spark.ui.enabled", String.valueOf(properties.isSparkUiEnabled()))
                .config("spark.sql.catalog." + properties.getIceberg().getCatalogName(), "org.apache.iceberg.spark.SparkCatalog")
                .config("spark.sql.catalog." + properties.getIceberg().getCatalogName() + ".type", "hadoop")
                .config("spark.sql.catalog." + properties.getIceberg().getCatalogName() + ".warehouse", properties.getIceberg().getWarehouse())
                .config("spark.sql.extensions", "org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions")
                .config("spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")
                .config("spark.hadoop.fs.s3a.path.style.access", properties.getS3().getPathStyleAccess());
    ...
    private void ensureIcebergTables(
        ...
        String namespaceName = properties.getIceberg().getCatalogName() + "." + properties.getIceberg().getNamespace();
        spark.sql("CREATE NAMESPACE IF NOT EXISTS " + namespaceName);

        String landingDdl = "CREATE TABLE IF NOT EXISTS " + landingTableName + " ("
                + "event_id STRING,"
                + "event_type STRING,"
            ...
            String tenantDimensionDdl = "CREATE TABLE IF NOT EXISTS " + tenantDimensionTable + " ("
                + "tenant_id STRING,"
            ...
            String eventTypeDimensionDdl = "CREATE TABLE IF NOT EXISTS " + eventTypeDimensionTable + " ("
                + "event_type STRING,"
                + "first_seen_at TIMESTAMP,"
                + "last_seen_at TIMESTAMP,"
                + "total_events BIGINT"
            ...
            String dailyFactDdl = "CREATE TABLE IF NOT EXISTS " + dailyFactTable + " ("
                + "tenant_id STRING,"
                + "event_date DATE,"
            ...
            spark.sql(
                "INSERT OVERWRITE " + tenantDimensionTable + " "
                    + "SELECT tenant_id, MIN(event_ts) AS first_seen_at, MAX(event_ts) AS last_seen_at, COUNT(*) AS total_events "
                    + "FROM " + landingTableName + " "
                    + "WHERE tenant_id IS NOT NULL "
                    + "GROUP BY tenant_id"
            );

            spark.sql(
                "INSERT OVERWRITE " + eventTypeDimensionTable + " "
                    + "SELECT event_type, MIN(event_ts) AS first_seen_at, MAX(event_ts) AS last_seen_at, COUNT(*) AS total_events "
                    + "FROM " + landingTableName + " "
            ...
            spark.sql(
                "INSERT OVERWRITE " + dailyFactTable + " "
